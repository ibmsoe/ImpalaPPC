...
static inline vector unsigned long long veclib_vec_cntlz_int128 (vector unsigned long long v)
{
  ...
  vector unsigned long long leading_zeros_per_half = 
    #ifdef __ibmxl__
      vec_cntlz
    #else
      /* gcc */
      vec_cntlz
    #endif
        ((vector unsigned long long) v);  /* 2 long long counts */
...
VECLIB_INLINE unsigned long long vec_crc321ud (unsigned long long crc, unsigned long long next)
{
  ...
  vector unsigned char vuc = veclib_int96_crc_mod2 ((vector unsigned char) merged); 
  __m128i_union int96_crc_mod2 = (__m128i_union) vuc;
