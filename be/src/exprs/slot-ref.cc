...
Status SlotRef::GetCodegendComputeFn(RuntimeState* state, llvm::Function** fn) {
  ...
  *fn = CreateIrFunctionPrototype(codegen, "GetSlotRef", &args);
  ...
  if (type_.IsStringType()) {
    Value* ptr_ptr = builder.CreateStructGEP(nullptr, val_ptr, 0, "ptr_ptr");
    ptr = builder.CreateLoad(ptr_ptr, "ptr");
    Value* len_ptr = builder.CreateStructGEP(nullptr, val_ptr, 1, "len_ptr");
    len = builder.CreateLoad(len_ptr, "len");
  } else if (type() == TYPE_TIMESTAMP) {
    Value* time_of_day_ptr = builder.CreateStructGEP(nullptr, val_ptr, 0, "time_of_day_ptr");
    ...
    Value* date_ptr = builder.CreateStructGEP(nullptr, val_ptr, 1, "date_ptr");
