...
Function* OldHashTable::CodegenHashCurrentRow(RuntimeState* state) {
  ...
  if (var_result_begin_ == -1) {
    ...
    if (results_buffer_size_ > 0) {
      ...
      hash_result = builder.CreateCall(hash_fn, {data, len, hash_result});
    }
  } else {
    if (var_result_begin_ > 0) {
    ...
    for (int i = 0; i < build_expr_ctxs_.size(); ++i) {
      ...
      if (stores_nulls_) {
        ...
        str_null_result = builder.CreateCall(null_hash_fn, {llvm_loc, len, hash_result});
        builder.CreateBr(continue_block);
      ...
      Value* ptr = builder.CreateStructGEP(nullptr, str_val, 0, "ptr");
      Value* len = builder.CreateStructGEP(nullptr, str_val, 1, "len");
      ...
      Value* string_hash_result =
          builder.CreateCall(general_hash_fn, {ptr, len, hash_result});
