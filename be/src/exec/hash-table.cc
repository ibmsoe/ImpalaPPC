...
static void CodegenAssignNullValue(LlvmCodeGen* codegen,
  ...
  if (type.type == TYPE_STRING || type.type == TYPE_VARCHAR) {
    Value* dst_ptr = builder->CreateStructGEP(nullptr, dst, 0, "string_ptr");
    Value* dst_len = builder->CreateStructGEP(nullptr, dst, 1, "string_len");
  ...
  *fn = prototype.GeneratePrototype(&builder, &this_arg);
  ...
  if (var_result_begin_ == -1) {
    ...
    if (results_buffer_size_ > 0) {
      ...
      hash_result = builder.CreateCall(hash_fn, {data, len, hash_result}, "hash");
    }
  } else {
    if (var_result_begin_ > 0) {
      Function* hash_fn = use_murmur ?
    ...
    for (int i = 0; i < build_expr_ctxs_.size(); ++i) {
      ...
      if (stores_nulls_) {
        ...
        str_null_result =
            builder.CreateCall(null_hash_fn, {llvm_loc, len, hash_result}, "str_null");
        builder.CreateBr(continue_block);
      ...
      Value* ptr = builder.CreateStructGEP(nullptr, str_val, 0);
      Value* len = builder.CreateStructGEP(nullptr, str_val, 1);
      ...
      Value* string_hash_result =
          builder.CreateCall(general_hash_fn, {ptr, len, hash_result}, "string_hash");
