...
Function* TextConverter::CodegenWriteSlot(LlvmCodeGen* codegen,
  ...
  if (check_null) {
    if (is_default_null) {
      is_null = builder.CreateCall(is_null_string_fn, {args[1], args[2]});
    } else {
      is_null = builder.CreateCall(is_null_string_fn, {args[1], args[2],
  ...
  Value* slot = builder.CreateStructGEP(nullptr, args[0], slot_desc->field_idx(), "slot");

  if (slot_desc->type().IsVarLenStringType()) {
    Value* ptr = builder.CreateStructGEP(nullptr, slot, 0, "string_ptr");
    Value* len = builder.CreateStructGEP(nullptr, slot, 1, "string_len");
    ...
    Value* result = builder.CreateCall(parse_fn, {args[1], args[2], parse_result_ptr});
