...
HADOOP_CLIENT_OPTS="-Xmx2024m -XX:MaxPermSize=512M" hive --service metastore -p $HIVE_METASTORE_PORT \
...
if [ ${ONLY_METASTORE} -eq 0 ]; then
  ...
  HADOOP_CLIENT_OPTS="-XX:MaxPermSize=512M" hive --service hiveserver2 > ${LOGDIR}/hive-server2.out 2>&1 &
